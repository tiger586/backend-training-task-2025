...
var l = '8'
// 請文字解釋為什麼
console.log(k * l > 21) // true，因為相乘，javascript 會自動轉成數字
...
if (performanceData.coaches[0].performance < 50000) {
  // 請填寫第一位教練業績增長程式碼，使用 +=
  // 計算未達成目標還有多少
  notAchieved[0] = 50000 - performanceData.coaches[0].performance
  performanceData.coaches[0].performance += notAchieved[0]
}
...
if (performanceData.coaches[1].performance < 50000) {
  // 請填寫第二位教練業績增長程式碼，使用 +=
  // 計算未達成目標還有多少
  notAchieved[1] = 50000 - performanceData.coaches[1].performance
  performanceData.coaches[1].performance += notAchieved[1]
}
...
// 練習：計算小明今日消耗的卡路里
calorieBurn += activities.biking.calorie * 10
calorieBurn += activities.running.calorie * 30
calorieBurn += activities.yoga.calorie * 40
calorieBurn += activities.biking.calorie * 10
...
if (exerciseRecords[3].duration >= 30 && exerciseRecords[3].heartRate >= 130) {
  let wednesdayDuration = exerciseRecords[3].duration
  totalDuration += wednesdayDuration
  validDays += 1
}
